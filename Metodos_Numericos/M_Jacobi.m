fprintf('Tabla de iteraciones:\n');
         while norma > criterio
            k=k+1;          Tabla(k,:) = k;
            if(k >= z)      %Paso 6 Compruebo si cumplo con el maximo número de iteraciones
           ...
           norma = norma^(1/2);      %Obtengo la norma
         ...
         if (norma < criterio)
            fprintf('\nResultado:\n');
            for i=1:numEc
                fprintf('X(%d) = %0.4f\n',i,X0(i));    %Imprimo Resultado
            end
            plot(errorArreglo,'-r');
            title('Gráfica de la Norma "Jacobi"')                             
         end
         fprintf('\n');